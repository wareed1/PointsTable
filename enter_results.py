...
def check_age():
    ...
    if len(parts) == 3:
        ...
        matched_age = (
            parts[1] == age_category_var.get()
            or (parts[1] == "Boys 8 & Under" and age_category_var.get() == "Boys 6 & Under")
            or (parts[1] == "Girls 8 & Under" and age_category_var.get() == "Girls 6 & Under")
            or (parts[1] == "Boys 8 & Under" and age_category_var.get() == "Boys 7 & 8")
            or (parts[1] == "Girls 8 & Under" and age_category_var.get() == "Girls 7 & 8")
        )
        if not matched_age:
            tkMessageBox.showinfo(
                "!!! WARNING !!!",
                "Age category mismatch for swimmer:  " + parts[1] + " != " + age_category_var.get(),
                parent=master,
            )
...
def generate_results():
    ...
    if results_warning == "True":
        if return_code != 0:
            tkMessageBox.showinfo(
                "Error",
                "Results generation failed, check Command Prompt window for details",
                parent=master,
            )
...
def validate_data_entry():
    ...
    missing_field = (
        not lane_entry.get()
        or not minutes_entry.get()
        or not seconds_entry.get()
        or not swimmer_number_entry.get()
        or not event_var.get()
        or not swimmer_name_entry.get()
        or swimmer_name_entry.get() == "NOT FOUND"
        or not valid_var.get()
        or not age_category_var.get()
        or not club_var.get()
        or not class_var.get()
    )
    if missing_field:
...
def save_results():
    ...
    if duplicate:
        tkMessageBox.showinfo(
            "Error", "Record already exists for this swimmer in this event", parent=master
        )
        return
    line = [
        parts[0],
...
def get_next_available_swimmer_number_for_club(club):
    ...
    if not swimmers_data:
        # empty list so start at 1
        return 1, 0
    while not done and index <= len(swimmers_data):
        ...
        if swimmers_data[index - 1][2] == club:
            if len(swimmers_data) == index:
                # next swimmer number is one more than current entry
                swimmer_number = int(swimmers_data[index - 1][0]) + 1
                done = True
            elif int(swimmers_data[index][0]) > int(swimmers_data[index - 1][0]) + 1:
                # found a free space
                swimmer_number = int(swimmers_data[index - 1][0]) + 1
                done = True
        swimmer_number = int(swimmers_data[index - 1][0]) + 1
...
def show_next_club_swimmer(club):
    ...
    if swimmer_number == -1:
        ...
        tkMessageBox.showinfo(
            "Error",
            "Could not find an available swimmer number for the selected club",
            parent=master,
        )
        return False
    # update screen
...
def change_mode_to_clubs_entry():
    ...
    club = club_var.get()
    if club == "":
        tkMessageBox.showinfo("Error", "Set club before entering this entry mode", parent=master)
        # reset to event mode
...
def cb_set_swimmer_number(event):
    """ Change the swimmer number in Club entry mode """
    # must be in Club mode for this features
    if entry_mode_var.get() != ENTRY_MODE_LIST[2]:
        tkMessageBox.showinfo("Error", "This feature only works in Club mode")
        swimmer_name_entry.focus()
        return
    done = False
    current_swimmer_number = swimmer_number_entry.get()
    while not done:
        user_selected = simpledialog.askstring(
            "Input", "Enter the next swimmer number:", parent=master
        )
        if not user_selected or user_selected == "":
            # user is quitting, cancel feature
            swimmer_name_entry.focus()
            return
        if user_selected <= current_swimmer_number:
            tkMessageBox.showinfo("Error", "Invalid swimmer number, try again")
            continue
        done = True
    swimmer_number_entry.focus()
    swimmer_number_entry.delete(0, END)
    swimmer_number_entry.insert(END, user_selected)
    swimmer_name_entry.focus()
    ...
    if swimmer_number != "" and event_number != "":
        ...
        if swimmer_idx == -1:
            ...
            if results_idx == -1:
                tkMessageBox.showinfo(
                    "Error",
                    "No matching event for the swimmer number: " + swimmer_number,
                    parent=master,
                )
            else:
...
Label(master, textvariable=next_available_var, font=(FONT, FONT_SIZE), anchor="e").grid(
    row=NEXT_AVAILABLE_ROW, column=NEXT_AVAILABLE_COLUMN, sticky="W"
)
...
Label(master, text=SWIMMER_NUMBER_LABEL_TEXT, font=(FONT, FONT_SIZE), anchor="e").grid(
    row=SWIMMER_NO_ROW, column=LABEL_COLUMN, sticky="E", padx=4, pady=4
)
...
Label(master, text=EVENT_LABEL_TEXT, font=(FONT, FONT_SIZE), anchor="e").grid(
    row=EVENT_ROW, column=LABEL_COLUMN, sticky="E", pady=4
)
event_var = StringVar()
if first_event == "True":
...
event_dropdown.grid(
    row=EVENT_ROW, column=PRIME_ENTRY_COLUMN, columnspan=2, sticky="W", padx=4, pady=4
)
...
Label(master, text=LANE_LABEL_TEXT, anchor="e", font=(FONT, FONT_SIZE)).grid(
    row=LANE_ROW, column=LABEL_COLUMN, sticky="E", padx=4, pady=4
)
...
Label(master, text=TIME_LABEL_TEXT, anchor="e", font=(FONT, FONT_SIZE)).grid(
    row=TIME_ROW, column=LABEL_COLUMN, sticky="E", padx=4, pady=4
)
Label(master, text=MINUTES_REMINDER_LABEL_TEXT, anchor="w", font=(FONT, SMALL_FONT_SIZE)).grid(
    row=TIME_HINT_ROW, column=PRIME_ENTRY_COLUMN, sticky="WE"
)
Label(master, text=SECONDS_REMINDER_LABEL_TEXT, anchor="w", font=(FONT, SMALL_FONT_SIZE)).grid(
    row=TIME_HINT_ROW, column=SECOND_ENTRY_COLUMN, sticky="WE"
)
...
Label(master, text=VALID_LABEL_TEXT, anchor="e", font=(FONT, FONT_SIZE)).grid(
    row=VALID_ROW, column=LABEL_COLUMN, sticky="E", padx=4, pady=4
)
valid_var = StringVar()
...
save_entry_button = Button(
    master, textvariable=save_entry_button_var, command=save_results, font=(FONT, FONT_SIZE)
)
save_entry_button.grid(row=BUTTONS_1_ROW, column=SAVE_BUTTON_COLUMN, sticky="W", pady=4)

# Generate results button on screen
Button(
    master, text=GENERATE_RESULTS_BUTTON_TEXT, command=generate_results, font=(FONT, FONT_SIZE)
).grid(row=BUTTONS_1_ROW, column=GENERATE_BUTTON_COLUMN, sticky="W", pady=4)
...
Label(master, text=SWIMMER_NAME_LABEL_TEXT, font=(FONT, FONT_SIZE), anchor="e").grid(
    row=SWIMMER_NAME_ROW, column=LABEL_COLUMN, sticky="E", padx=4, pady=4
)
...
Label(master, text=AGE_CATEGORY_LABEL_TEXT, font=(FONT, FONT_SIZE), anchor="e").grid(
    row=CATEGORY_ROW, column=LABEL_COLUMN, sticky="E", padx=4, pady=4
)
...
Label(master, text=CLUB_LABEL_TEXT, font=(FONT, FONT_SIZE), anchor="e").grid(
    row=CLUB_ROW, column=LABEL_COLUMN, sticky="E", padx=4, pady=4
)
club_var = StringVar()
...
Label(master, text=CLASS_LABEL_TEXT, font=(FONT, FONT_SIZE), anchor="e").grid(
    row=CLASS_ROW, column=LABEL_COLUMN, sticky="E", padx=4, pady=4
)
CLASS_LIST = ["N", "C", "T"]
class_var = StringVar()
...
Button(master, text=SAVE_SWIMMTER_BUTTON_TEXT, command=save_swimmer, font=(FONT, FONT_SIZE)).grid(
    row=BUTTONS_2_ROW, column=SAVE_BUTTON_COLUMN, sticky="W", pady=4
)

# Quit button on screen
Button(master, text=QUIT_BUTTON_TEXT, command=master.quit, font=(FONT, FONT_SIZE)).grid(
    row=BUTTONS_2_ROW, column=QUIT_BUTTON_COLUMN, sticky="E", pady=4
)

# Help guidance on screen
help_text = (
    "F1 for team list\n"
    + "F2 to set swimmer number in Clubs mode\n"
...
Label(master, text=help_text, anchor="e", font=(FONT, SMALL_FONT_SIZE)).grid(
    row=HELP_TEXT_ROW, column=HELP_TEXT_COLUMN, sticky="W", padx=4, pady=4
)

# Entry mode selection on screen
Label(master, text=ENTRY_MODE_LABEL_TEXT, font=(FONT, SMALL_FONT_SIZE), anchor="ne").grid(
    row=ENTRY_MODE_TEXT_ROW, column=ENTRY_MODE_TEXT_COLUMN, sticky="E", padx=4, pady=4
)
entry_mode_var = StringVar()
entry_mode_dropdown = OptionMenu(
    master, entry_mode_var, *ENTRY_MODE_LIST, command=cb_ready_entry_mode
)
...
Button(
    master, text=RELOAD_SWIMMERS_BUTTON_TEXT, command=reload_swimmers, font=(FONT, FONT_SIZE)
).grid(row=BUTTONS_2_ROW, column=RELOAD_SWIMMERS_BUTTON_COLUMN, sticky="W", pady=4)

# bind special keys to functions
master.bind("<Return>", cb_reposition_cursor)
master.bind("<F1>", cb_list_teams)
master.bind("<F2>", cb_set_swimmer_number)
